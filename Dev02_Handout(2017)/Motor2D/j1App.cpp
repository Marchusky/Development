...
bool j1App::LoadGame()
{
	...
	// Open document & check if all good
	pugi::xml_parse_result result = loadgame_file.load_file("loadgame.xml");
	if (result == NULL)
	{
		LOG("Could not load map xml file loadgame.xml. pugi error: %s", result.description());
		ret = false;
	}
	else
	{
		loading = loadgame_file.child("config");
		app_loadgame = loading.child("app");
	}
	...
	if (ret == true)
	{
		...
		while (item != NULL && ret == true)
		{
			ret = item->data->Awake(loading.child(item->data->name.GetString()));
			item = item->next;
		}
	}
	//return false to not do an infinite loop
	return false;
}
